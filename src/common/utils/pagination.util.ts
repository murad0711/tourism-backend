...
export async function paginate<T extends ObjectLiteral>(
  ...
  let sortBy = paginationDto.sortBy || 'createdAt';
  let sortOrder = paginationDto.sortOrder || SortOrder.DESC;

  if (paginationDto.sort) {
    if (paginationDto.sort.startsWith('-')) {
      sortBy = paginationDto.sort.substring(1);
      sortOrder = SortOrder.DESC;
    } else {
      sortBy = paginationDto.sort;
  ...
  if (paginationDto.search && searchFields.length > 0) {
    queryBuilder.andWhere(
      `(${searchFields.map((field) => `${field} ILIKE :search`).join(' OR ')})`,
